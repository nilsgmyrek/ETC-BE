...
output$new_scientificName <- sapply(output$new_scientificName, function(x) {
  if (length(x) == 0) return(NA)  # Handle empty elements
  return(paste(x, collapse = "; "))  # Join multiple values with a semicolon
})
